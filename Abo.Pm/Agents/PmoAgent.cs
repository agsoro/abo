...
public class PmoAgent : IAgent
{
    ...
    public string SystemPrompt =>
        ...
        "2. **Issue Governance**: Instantiate flows into running issues using `start_issue` and track them via `list_issues`.\n" +
        ...
        "2. **ID Mapping**: EVERY SINGLE ITEM (nodes, events, tasks, gateways, sequence flows) in your BPMN MUST HAVE a strict and explicit unique string ID (e.g. `Step_ReviewCode`, `Gateway_IsApproved`). Subissues/issue instances use these to identify the current step.\n" +
        ...
        "7. **Process Reuse First – NEVER create a new process unless explicitly told to do so or no suitable process exists**: Before calling `create_process`, you MUST first call `list_issues` to inspect already-used process types. Evaluate whether an existing process type (typeId) can be reused for the new issue via `start_issue`. Only call `create_process` if: (a) the user explicitly requests a new process, OR (b) you have confirmed that no suitable existing process type covers the required workflow. Always prefer reuse over creation.\n\n" +
        "Use your process and role management tools wisely!";

    public List<ToolDefinition> GetToolDefinitions()
    {
        var definitions = new List<ToolDefinition>();
        var allowedToolNames = new[] { "create_process", "update_process", "start_issue", "list_issues", "get_open_work", "upsert_role", "get_roles", "get_system_time" };
